...
findTop <- function(vect, topX=10) {
  topX = min(topX,ceiling(length(vect)/30))
  mean(order(vect, decreasing=T)[1:topX])
}

findBottom <- function(vect, topX=10){
  topX = min(topX,ceiling(length(vect)/30))
  mean(order(vect, decreasing=F)[1:topX])
}


#' compute peak time, updated version of caoGenes
#'
#' compute peak time
#' 
#' @param expSmooth smoothed expression matrix
#' @param expDat expression matrix
#' @param sampTab sample table
#' @param dynRes result of running findDynGenes
#' @param pThresh pval threshold
#'
#' @return data.frame of dynamically expressed genes, peakTime, valleyTime, ordered by peaktime
#' @export
#'
computePT<-function(
  expSmooth,
  ...
  # find max peak from smoothed data
  genesPeakTimes = apply(expSmooth[genes,], 1, which.max)
  # estimate max peak from raw data
  expDat = expDat[,colnames(expSmooth)]     # order columns along pseudotime to match expSmooth
  genesPeakTimesRaw = apply(expDat[genes,], 1, findTop)

  # find min valleys
  genesValleyTimes = apply(expSmooth[genes,], 1, which.min)
  genesValleyTimesRaw = apply(expDat[genes,], 1, findBottom)

  summary<-data.frame(genesPeakTimes)
  summary<-cbind(summary,data.frame(genesPeakTimesRaw))
  summary<-cbind(summary,data.frame(genesValleyTimes))
  summary<-cbind(summary,data.frame(genesValleyTimesRaw))

  # ^^ these find which cells are max, but NOT corresponding pseudotime
  summary$PTmax<-sampTab[colnames(expSmooth)[summary$genesPeakTimes],"pseudotime"]
  summary$PTmin<-sampTab[colnames(expSmooth)[summary$genesValleyTimes],"pseudotime"]

  # order by peak times
  geneDF<-summary[order(summary$PTmax,decreasing=FALSE),]
...
#' add dist and weight
#'
#' add dist and weight to GRN, new version of PT weighting
#'
#'
#' @param grnTab grn table
#' @param geneDF from running computePT
#' @param ncells number of cells
#' @param maxNeg threshold for setting adjusted_weight to 0 
#'
...
weightPT<-function(
  ...
  if (use_order){
    maxdist<-ncol(sampTab)
    if (use_raw){
      geneDF$PTmax<-geneDF$genesPeakTimesRaw
      geneDF$PTmin<-geneDF$genesValleyTimesRaw
    }else{
      geneDF$PTmax<-geneDF$genesPeakTimes
      geneDF$PTmin<-geneDF$genesValleyTimes
    }
  }else{
    maxdist<-max(sampTab$pseudotime)-min(sampTab$pseudotime)
  }

  # separate GRN by activation or repression
  pos_grnTab<-grnTab[grnTab$corr>=0,]
  neg_grnTab<-grnTab[grnTab$corr<0,]

  # compute normdists for positive regulation as peak difference
  tf<-as.vector(pos_grnTab$TF)
  tg<-as.vector(pos_grnTab$TG)
  pt_dist<-geneDF[tg,]$PTmax - geneDF[tf,]$PTmax
  pt_dist<-pt_dist/maxdist

  pos_grnTab<-cbind(pos_grnTab,normdist=pt_dist)

  # compute normdists for negative regulation as valley time - peak time
  tf<-as.vector(neg_grnTab$TF)
  tg<-as.vector(neg_grnTab$TG)
  pt_dist<-geneDF[tg,]$PTmin - geneDF[tf,]$PTmax
  pt_dist<-pt_dist/maxdist

  neg_grnTab<-cbind(neg_grnTab,normdist=pt_dist)

  new_grnTab<-rbind(pos_grnTab,neg_grnTab)

  distscores<-distScore(new_grnTab$normdist,trxLag=trxLag, maxNeg=maxNeg)
  new_grnTab<-cbind(new_grnTab, adjusted_weight = new_grnTab$zscore * distscores)
